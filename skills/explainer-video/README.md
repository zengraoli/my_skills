...
**在 agent 里用自然语言说主题即可，不需要记参数。** agent 会读 `SKILL.md` 自己走七步：检查环境 → 联网调研 → 写分场文稿 → 做幻灯片 → 截图自检 → 合成 → 把 mp4 路径和事实来源给你。

```
帮我做一期讲 /loop 命令的视频
按上次那样出个视频，主题是 GPT-6 Astra 的能力，竖屏
```

- Claude Code：直接说，或 `/explainer-video 主题` 强制触发。
- Codex / Kimi Code / grok-cli 等：装好 `.agents/skills/` 后直接说需求。
- 想改音色、横竖屏、语速，直接在需求里说，agent 会传对应参数。
