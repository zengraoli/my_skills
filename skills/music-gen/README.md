...
**在 agent 里用自然语言说需求即可，不需要记参数。** agent 会读 `SKILL.md` 自己走流程：检查环境 → 写词 → 生成 → 按需修前奏 → 把文件路径给你。

```
帮我写一首古风伤感的歌，女声，前奏别太长，开头加一句"送给所有等过的人"
给这个视频配一段 2 分钟的 Lo-fi 纯音乐
这首歌前奏太长了，截到 5 秒
```

- Claude Code：直接说，或 `/music-gen 写一首……` 强制触发。
- Codex / Kimi Code / grok-cli 等：装好 `.agents/skills/` 后直接说需求，agent 会匹配到本 skill。
- 想知道有哪些可调项，问 agent"这个歌曲生成有哪些参数可以调"，它会按 SKILL.md 回答。
...
- 高潮处的群唱感：副歌末句在歌词里连写两遍 + `--style` 写现场/万人合唱，第二遍会明显变厚（实测有效，但只是"人变多了"，不是独立的观众声部）。
...
| **歌手唱一句、观众接一句 / 现场版** | 只能"氛围 + 重复句变厚" | 没有 `[crowd]`、`[audience]` 之类的标签语义，写了会被唱出来。能做到的上限是：副歌末句连写两遍 + 现场 style，第二遍会像多人合唱；但"歌手一句、观众一句"的交替、以及指定哪句由观众唱，都做不到。用 `additional_prompt` 硬指派"第二遍换观众"反而让整首人声糊掉 |
| **模型内的开场独白** | 不稳定 | 把话写进 `[intro]` 模型会用说话方式念，但有重音/吞字，念完还会铺十几秒前奏。所以本 skill 改用 TTS + ffmpeg 在歌外面做，可控 |
| **同一首歌翻新编曲 / 续写** | 没有 | 每次生成都是新歌，旋律会变；没有"保留旋律换编曲"或"在这首歌基础上续写"的接口 |
